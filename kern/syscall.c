	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((rsrc=envid2env(srcenvid,&esrc,1)) < 0
			|| (rdst=envid2env(dstenvid,&edst,1)) < 0)
	{
		return -E_BAD_ENV;
	}
	...
	struct PageInfo * pp;
	pte_t *srcpte;
	if ( !( pp=page_lookup(esrc->env_pgdir,srcva,&srcpte) ) ){
		return -E_INVAL;
	}
	if( (perm & (PTE_U | PTE_P)) != (PTE_U | PTE_P)
			|| (perm & (~PTE_SYSCALL)) ){ 
	...
	if ( (rdst = page_insert(edst->env_pgdir,pp,dstva,perm))<0 ){
		return rdst;
	}
	return 0;
